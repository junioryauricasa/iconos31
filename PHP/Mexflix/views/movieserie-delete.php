...
if ( $_POST['r'] == 'movieserie-delete' && !isset($_POST['crud']) ) {
	...
	if( empty($ms) ) {
		$template = '
			<div class="container">
				<p class="item error">No existe el imdb_id <b>%s</b></p>
			</div>
			<script>
...
} else if ($_POST['r']=='movieserie-delete' && $_POST['crud'] == 'del') {
	$ms = $ms_controller->del($_POST['imdb_id']);

	$template = '
		<div class="container">
			<p class="item delete">
				MovieSerie <b>%s</b> eliminada
			</p>
		</div>
		<script>
			window.onload = function () {
